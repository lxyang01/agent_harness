...
每一层防线都可独立验证:**22 条对抗探针**(零费用、确定性)全部防御成功,162 项单元测试覆盖并发竞态、权限边界、数据隔离与记忆分层。
...
tests/                 # 162 项:并发竞态/隔离/契约/审批链路/记忆分层(llm_doubles 脚本模型)
...
当前 **162 项**:并发竞态专项、身份与隔离、运行时加固、记忆分层(脚本模型驱动)。
