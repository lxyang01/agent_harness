...
本分支在集群形态下保留全部防线,并把并发防御重建在分布式原语上(Redis 会话锁、全集群 LLM 限额、PG 行锁审批恰好一次、Redis 登录失败计数、同源校验 CSRF 防线、MCP 熔断降级),278 项单元测试与 25 条对抗探针在 PG/Redis 后端下全部通过(宿主机与集群内两端验证)。
...
- **跨站请求防护(CSRF)**:所有 POST(含登录)校验 Origin/Referer 与 Host 同源,伪造跨站请求 403(adv-025 探针验证;非浏览器客户端无该头则放行,权衡见 `docs/security-baseline.md`);https 部署可开 `BILLGUARD_SECURE_COOKIES` 为 Cookie 加 Secure 属性
- **MCP 熔断与降级**:MCP 连接断开按 1s/2s/4s 指数退避重连,三次失败熔断 60 秒快速失败,对话降级为"服务暂时不可用"话术而非报错;半开探测自动恢复;高危写操作降级不会被记为已执行(审计诚实,细节见 `docs/concurrency-guarantees.md`)
