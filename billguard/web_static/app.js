...
function renderSubscriptions() {
  ...
  $("#subscription-list").innerHTML = items.length ? items.map((sub) => `<div class="bar-row"><span title="${escapeHtml(sub.name)}">${escapeHtml(sub.name)}<small>${sub.cycle} · ${escapeHtml(sub.merchant)}</small></span><div class="bar-track"><div class="bar-fill" style="width:${sub.active?100:0}%"></div></div><strong>${formatAmount(sub.expected_amount)}</strong></div>`).join("") : '<div class="empty-chart">还没有订阅记录</div>';
}
...
function renderCategories() {
  ...
  $("#category-audit-list").innerHTML=state.audits.length?state.audits.slice(0,20).map(item=>`<div class="audit-item"><span class="audit-dot"></span><div><strong>${escapeHtml(actionLabels[item.action]||item.action)}${item.tx_id?` · ${escapeHtml(item.tx_id)}`:""}</strong><small>${escapeHtml(item.operator)} · ${escapeHtml(formatDate(item.changed_at))}</small></div></div>`).join(""):'<div class="audit-empty">暂无审计记录</div>';
}
...
function renderWorkflowAudits(audits){$("#workflow-audits").innerHTML=audits.length?audits.map(item=>{let changes={};try{changes=JSON.parse(item.new_value);}catch(error){}let summary="";if(Object.prototype.hasOwnProperty.call(changes,"old_status")){summary=`状态：${changes.old_status||"—"} → ${changes.status||"—"}${changes.note?` · 备注：${changes.note}`:""}`;}else if(Object.prototype.hasOwnProperty.call(changes,"new_category")){summary=`类别：${changes.old_category||"—"} → ${changes.new_category}`;}else{summary=item.action;}return `<div class="audit-item"><span class="audit-dot"></span><div><strong>${escapeHtml(summary)}</strong><small>${escapeHtml(item.operator)} · ${escapeHtml(formatDate(item.changed_at))}</small></div></div>`;}).join(""):'<div class="audit-empty">暂无核查记录</div>';}
...
$("#apply-bulk").onclick=async()=>{const updates={};if($("#bulk-status").value)updates.status=$("#bulk-status").value;if($("#bulk-note").value.trim())updates.note=$("#bulk-note").value.trim();if(!Object.keys(updates).length){toast("请选择状态或填写备注");return;}try{const result=await api("/api/bills/workflow",{tx_ids:[...state.selected],updates});state.selected.clear();await loadBills(state.bills.page);toast(`已更新 ${result.count} 笔交易`);}catch(error){toast(error.message);}};
