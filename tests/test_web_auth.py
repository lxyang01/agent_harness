...
class WebAuthTestCase(StoreTestCase):
    """test_web_auth 公共底座:PG 存储 + Redis 协调的 BillGuardApp 装配。"""
    ...
    def build_app(self, llm=None, mcp_manager=None, policy_gateway=None,
                  work_item_store=None, authenticator=None) -> BillGuardApp:
        from tests.llm_doubles import FinalLLM
        return BillGuardApp(
            self.temp_root, self.temp_root / "docs", llm or FinalLLM(), mcp_manager,
            policy_gateway, work_item_store, authenticator,
...
def make_users(pool) -> PGUserStore:
...
class SessionOwnershipTests(WebAuthTestCase):
    def test_sessions_are_private_and_ownerless_admin_only(self):
        ...
        for method_args in (
            ...
            with self.assertRaises(PermissionDenied):
                method_args()
        # 归属留痕(原断言读本地会话文件;PG 等价可观察量:PGSessionStore 行)
        self.assertEqual("alice", PGSessionStore(self.pool).load("s-alice").owner)

    def test_ownerless_legacy_session_admin_only(self):
        app = self.build_app()
        store = PGSessionStore(self.pool)
        store.save(store.load("legacy"))  # 无主旧会话(owner=NULL 存量行)
        boss = User("admin", "admin")
        mallory = User("mallory", "user")
...
class ServerSideIdentityTests(WebAuthTestCase):
    ...
    def test_decide_approval_uses_server_identity_and_blocks_viewer(self):
        make_users(self.pool)
        from tests.llm_doubles import FinalLLM
        from billguard.storage_pg import PGWorkItemStore
        from billguard.policy import PolicyGateway
        ...
        app = self.build_app(ScriptedLLM(), FakeManager(), gateway, work_items)
        alice = User("alice", "user")
        mallory = User("mallory", "user")
...
class WorkflowOperatorKeyTests(WebAuthTestCase):
...
class UserDeleteTests(WebAuthTestCase):
    def _app(self):
        users = PGUserStore(self.pool)
        users.create("boss", "boss-pass-1234", "admin")
        users.create("alice", "alice-pass-123", "user")
        app = self.build_app(authenticator=Authenticator(users, RedisAuthSessions(self.redis)))
        return app, users
...
class PurgeMyDataTests(WebAuthTestCase):
...
class PurgeLegacyDataTests(WebAuthTestCase):
